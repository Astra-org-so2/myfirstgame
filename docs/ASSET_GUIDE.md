...
- Замена на импортированные ассеты (GLTF/textures/audio) — инкрементальна:
  ...
  (на ПК — dev-удобство), песочница-риг продолжает работать (файлы
  импорта — текстовые ссылки; отсутствующие `.ctex` в риге —
  fallback-материал + push_warning, не crash). **Mobile-требование
  (ADR-021):** текстуры — ASTC-совместимые форматы, размеры ≤ preset-
  лимитам (512/1024), atlasing для draw calls (TECH_DESIGN §12).
